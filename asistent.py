...
def open_game_01():
    ...
    score = 0
    score_label = tk.Label(window, text=f"Skóre: {score}", font=("Arial", 16))
    ...
    def klikni(btn_text, btn, index):
        ...
        if btn_text.get() != "":
            return
        nove = random.choice([0, 1])
        btn_text.set(str(nove))
        if nove == 0:
            status_label.config(text="❌ Prehral si!")
            ...
            if score == sum(1 for v in grid_values if v == 1):
                status_label.config(text="🏆 Vyhral si!")
                disable_buttons()
...
def open_coin_game():
    ...
    game.title("Hra s mincou")

    stav = 50
    historia = []

    label = tk.Label(game, text="Začínaš na 50", font=("Arial", 14))
    label.pack(pady=10)

    canvas = tk.Canvas(game, width=180, height=120)
    canvas.pack()

    hod_label = tk.Label(game, text="História hodov: -", font=("Arial", 12), wraplength=300, justify="left")
    hod_label.pack(pady=5)

    def otoc():
        nonlocal stav, historia
        minca = random.choice([-10, 10])
        stav += minca
        historia.append(minca)
        canvas.delete("all")
        canvas.create_oval(20, 20, 160, 160, fill="yellow")
        canvas.create_text(90, 50, text=f"Hod: {minca}", font=("Arial", 14, "bold"))
        canvas.create_text(90, 75, text=f"Stav: {stav}", font=("Arial", 12))
        if stav <= 0:
            label.config(text="Išiel si za školu!")
            button.config(state="disabled")
            reset_button.config(state="normal")
        elif stav >= 100:
            label.config(text="Si na vyučovaní!")
            button.config(state="disabled")
            reset_button.config(state="normal")
        else:
            label.config(text="Pokračuj...")
        hod_label.config(text="História hodov: " + ", ".join(map(str, historia)))

    def reset():
        nonlocal stav, historia
        stav = 50
        historia = []
        label.config(text="Začínaš na 50")
        hod_label.config(text="História hodov: -")
        button.config(state="normal")
        reset_button.config(state="disabled")
        canvas.delete("all")

    button = tk.Button(game, text="Otoč", command=otoc, font=("Arial", 14))
    button.pack(pady=5)
...
tk.Button(menu_frame, text="Seconds", command=open_converter, font=("Arial", 18), width=12, height=2).pack(pady=10, anchor="w")
tk.Button(menu_frame, text="Calculator", command=open_calculator, font=("Arial", 18), width=12, height=2).pack(pady=10, anchor="w")
tk.Button(menu_frame, text="Hra 0/1", command=open_game_01, font=("Arial", 18), width=12, height=2).pack(pady=10, anchor="w")
tk.Button(menu_frame, text="Hra s mincou", command=open_coin_game, font=("Arial", 18), width=12, height=2).pack(pady=10, anchor="w")
